...
public class PrgSuiviEvalAnneeRegionViewExcel extends AbstractXlsView {
    ...
    protected void buildExcelDocument(Map<String, Object> map, Workbook workbook, HttpServletRequest httpServletRequest, HttpServletResponse httpServletResponse) throws Exception {
        ...
        List<PrgSuiviEvalAnneeRegionView> paiements = (List<PrgSuiviEvalAnneeRegionView>)map.get("lesPaiements");
        ...
        for(PrgSuiviEvalAnneeRegionView paiement : paiements){
            Row paiementRow =  sheet.createRow(currentRow++);
            paiementRow.createCell(0).setCellValue(paiement.getAnneePeriode().intValue());
            paiementRow.createCell(1).setCellValue(paiement.getNomRegion());
